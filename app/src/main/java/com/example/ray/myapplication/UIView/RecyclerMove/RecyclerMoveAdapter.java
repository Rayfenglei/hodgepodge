...
public class RecyclerMoveAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements RecyclerMoveTouchInterface {
    // 我的频道 标题部分
    ...
    private List<RecyclerMoveItemBean> mMyChannelItems, mOtherChannelItems;

    public RecyclerMoveAdapter(Context context, ItemTouchHelper mItemTouchHelper, List<RecyclerMoveItemBean> mMyChannelItems, List<RecyclerMoveItemBean> mOtherChannelItems) {
    ...
    public void updateChannel(List<RecyclerMoveItemBean> selected, List<RecyclerMoveItemBean> unselected) {
        mMyChannelItems = selected;
        mOtherChannelItems = unselected;
        notifyDataSetChanged();
    }

    public boolean isEmpty() {
        return (mMyChannelItems == null || mMyChannelItems.isEmpty()) && (mOtherChannelItems == null || mOtherChannelItems.isEmpty());
    }

    public List<RecyclerMoveItemBean> getSeletedItem() {
        return mMyChannelItems;
    }

    public List<RecyclerMoveItemBean> getUnSeletedItem() {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(final ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case TYPE_OTHER:
                view = mInflater.inflate(R.layout.item_unselected_channel, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            ...
            myHolder.textView.setText(mMyChannelItems.get(position - COUNT_PRE_MY_HEADER).getItemName());
            if (isEditMode && !shouldDisableMove(myHolder)) {
            ...
            ((OtherViewHolder) holder).textView.setText(mOtherChannelItems.get(position - mMyChannelItems.size() - COUNT_PRE_OTHER_HEADER).getItemName());
    ...
    private void moveMyToOther(MyViewHolder myHolder) {
        ...
        RecyclerMoveItemBean item = mMyChannelItems.get(startPosition);
        ...
        if (mChannelItemClickListener != null) {
            mChannelItemClickListener.removeChannel(item);
        }
    }
    ...
    private int processItemRemoveAdd(OtherViewHolder otherHolder) {
        ...
        RecyclerMoveItemBean item = mOtherChannelItems.get(startPosition);
        mOtherChannelItems.remove(startPosition);
        mMyChannelItems.add(item);
        if (mChannelItemClickListener != null) {
            mChannelItemClickListener.addChannel(item);
        }
        return position;
    }
    ...
    public void onItemMove(int fromPosition, int toPosition) {
        RecyclerMoveItemBean item = mMyChannelItems.get(fromPosition - COUNT_PRE_MY_HEADER);
    ...
    private void startEditMode(RecyclerView parent) {
        ...
        for (int i = 0; i < visibleChildCount; i++) {
            ...
            View imgEdit = view.findViewById(R.id.img_edit);
            if (holder != null && !shouldDisableMove(holder) && imgEdit != null) {
    ...
    public static boolean shouldDisableMove(RecyclerView.ViewHolder holder) {
        return holder.getItemViewType() == RecyclerMoveAdapter.TYPE_MY && holder.getAdapterPosition() == 1;
    }

    public interface OnMyChannelItemClickListener {
        void onItemClick(View v, int position);
        void addChannel(RecyclerMoveItemBean channel);
        void removeChannel(RecyclerMoveItemBean channel);
        void onCnannelOrderChanged();
    }
    ...
    class MyViewHolder extends RecyclerView.ViewHolder implements OnDragVHListener {
        ...
        public void onItemFinish() {
            //textView.setBackgroundResource(R.drawable.bg_selected_channel);
            if (mChannelItemClickListener != null) {
                mChannelItemClickListener.onCnannelOrderChanged();
            }
        }
    }
