package com.example.ray.myapplication.DataParse.SQLite;
...
public class OrderMethod {
    ...
    public OrderMethod(Context context) {
    ...
    public void execSQL(String sql) {
        ...
        try {
            if (sql.contains("select")){
                Toast.makeText(context,"还没执行SELECT" , Toast.LENGTH_SHORT).show();
    ...
    public boolean deleteOrder(String name) {
        ...
        try {
            ...
            db.delete(OrderDBHelper.TABLE_NAME, "CustomName = ?", new String[]{name});
    ...
    public ArrayList<Order> getAllDatas () {
        SQLiteDatabase db = ordersDBHelper.getWritableDatabase();
        Cursor cs = db.rawQuery("select * from " + OrderDBHelper.TABLE_NAME, null);
        ArrayList<Order> datas = null;
        if(cs.moveToFirst()){
            datas = new ArrayList<>();
            for(cs.moveToFirst(); ! cs.isAfterLast(); cs.moveToNext()) {
                datas.add(new Order(cs.getInt(cs.getColumnIndex("Id")),
                        cs.getString(cs.getColumnIndex("CustomName")),
                        cs.getInt(cs.getColumnIndex("OrderPrice")),
                        cs.getString(cs.getColumnIndex("Country"))));
            }
        }
        cs.close();
        return datas;
